...
public class AlbastruScurtComplet extends OpMode {
    ...
    private void buildPaths() {
        ...
        pathChain = follower.pathBuilder()
            ...
            // Path 9: 143.5° → 180° (36.5° rotation)
            .addPath(new BezierLine(
                new Pose(44.473, 94),
                new Pose(43.720, 35.656)
            ))
            .setLinearHeadingInterpolation(Math.toRadians(143.5), Math.toRadians(180))
            .setConstraints(straightConstraints)

            // Path 10: Move left (no rotation)
            .addPath(new BezierLine(
                new Pose(43.720, 35.656, Math.toRadians(180)),
                new Pose(16.376, 35.344, Math.toRadians(180))
            ))
            .setConstantHeadingInterpolation(Math.toRadians(180))
            .setConstraints(straightConstraints)

            // Path 11: Final path 180° → 112° (68° rotation)
            .addPath(new BezierCurve(
                new Pose(16.376, 35.344),
                new Pose(30, 28),
                new Pose(43, 21),
                new Pose(56, 15)
            ))
